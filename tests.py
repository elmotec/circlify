...
class CircleOutputTest(unittest.TestCase):
    ...
    def test_repr_and_str_with_id_(self):
        ...
        expected = "Circle(level=1, datum=2.0, x=0.0, y=0.0, r=0.0, " \
                   "ex={'label': '3'})"
        actual = str(circ.Circle(1, 2.0, ex={'label': '3'}))
        self.assertEqual(expected, actual)

    def test_repr_and_str_with_circle(self):
        """Check str() and repr() conversion"""
        expected = "Circle(level=1, datum=2.0, x=0.0, y=0.0, r=1.0, ex=None)"
        self.assertEqual(expected, str(circ.Circle(1, 2.0, r=1.0)))
...
class SpecialCases(unittest.TestCase):
    ...
    def test_single_value(self):
        ...
        expected = [circ.Circle(1, 2.0, r=1.0)]
        self.assertEqual(actual, expected)

    def test_two_equal_values(self):
        """Two equal circle cases is also trivial."""
        actual = circ.circlify([1.0, 1.0])
        expected = [circ.Circle(1, 1.0, x=0.5, r=0.5),
                    circ.Circle(1, 1.0, x=-0.5, r=0.5)]
        self.assertEqual(actual, expected)
...
class PrimeSerieTestCase(TestCaseWithDisplay):
    ...
    def test_circlify(self):
        ...
        expected = [
            circ.Circle(0, x=0.0, y=0.0, r=1.0),
            circ.Circle(1, 1, x=0.09222041925800777, y=0.8617116738294696,
                        r=0.09068624109026069),
            circ.Circle(1, 2, x=-0.40283175658099674, y=0.7512387781681531,
                        r=0.12824971207048294),
            circ.Circle(1, 3, x=0.3252787490004198, y=0.7776370388468007,
                        r=0.15707317711577193),
            circ.Circle(1, 5, x=0.48296614887228806, y=0.4541723195782383,
                        r=0.20278059970175755),
            circ.Circle(1, 7, x=-0.6132109517981927, y=0.4490810687795324,
                        r=0.23993324126007678),
            circ.Circle(1, 11, x=-0.045884607890591435, y=-0.6977206243364218,
                        r=0.3007722353441051),
            circ.Circle(1, 13, x=-0.04661299415374866, y=0.4678014425767657,
                        r=0.32697389223002427),
            circ.Circle(1, 17, x=-0.411432317820337, y=-0.13064957525245907,
                        r=0.3739089508053733),
            circ.Circle(1, 19, x=0.35776879346704843, y=-0.13064957525245907,
...
class CountSerieTestCase(TestCaseWithDisplay):
    ...
    def test_circlify(self):
        ...
        expected = [
            circ.Circle(0, x=0.0, y=0.0, r=1.0),
            circ.Circle(1, 2, x=-0.7680630545906644, y=0.13661056172475666,
                        r=0.21988250795031175),
            circ.Circle(1, 3, x=-0.4586184780594718, y=0.5154819840108337,
                        r=0.2692999739208646),
            circ.Circle(1, 4, x=0.15910532107887837, y=-0.6704181394216174,
                        r=0.31096082487194077),
            circ.Circle(1, 5, x=0.15769153632817096, y=0.5438978793053209,
                        r=0.34766477137653345),
            circ.Circle(1, 6, x=-0.20976457776763055, y=-0.08515409741642607,
                        r=0.3808476754656075),
            circ.Circle(1, 7, x=0.5824456027453089, y=-0.08515409741642607,
...
class GeometricSerieTestCase(TestCaseWithDisplay):
    ...
    def test_circlify(self):
        ...
        expected = [
            circ.Circle(0, x=0.0, y=0.0, r=1.0),
            circ.Circle(1, 16, x=-0.18045635173699437, y=-0.22990093891844118,
                        r=0.051776695296636886),
            circ.Circle(1, 32, x=-0.42762855855875753, y=-0.6656611405645538,
                        r=0.07322330470336313),
            circ.Circle(1, 64, x=-0.5170830169797044, y=-0.5131885205024093,
                        r=0.10355339059327377),
            circ.Circle(1, 128, x=0.10281914590763144, y=-0.662720719883036,
                        r=0.14644660940672627),
            circ.Circle(1, 256, x=-0.20710678118654763, y=-0.49258571550470814,
                        r=0.20710678118654754),
            circ.Circle(1, 512, x=-0.2218254069479773, y=0.6062444788590926,
                        r=0.29289321881345254),
            circ.Circle(1, 1024, x=-0.5857864376269051, y=0.0,
                        r=0.4142135623730951),
            circ.Circle(1, 2048, x=0.4142135623730951, y=0.0,
...
class MultiLevelInputTestCase(TestCaseWithDisplay):
    ...
    def test_json_input(self):
        ...
        actual = circ.circlify(self.data, show_enclosure=True)
        expected = [
            circ.Circle(level=0, datum=None, x=0.0, y=0.0, r=1.0),
            circ.Circle(level=1, datum=0.05, x=-0.565803075997749,
                        y=0.41097786651145324, r=0.18469903125906464),
            circ.Circle(level=1, datum=0.05, ex={'id': 'a2'},
                        x=-0.3385727489559141, y=0.7022188441650276,
                        r=0.18469903125906464),
            circ.Circle(level=1, datum=0.1, ex={'id': 'a1'},
                        x=-0.7387961250362587, y=0.0, r=0.2612038749637415),
            circ.Circle(level=1, datum=0.8, ex={'id': 'a1'},
                        x=0.2612038749637414, y=0.0, r=0.7387961250362586),
            circ.Circle(level=2, datum=0.01, x=-0.7567888163564136,
                        y=0.14087823651338607, r=0.0616618704777984),
            circ.Circle(level=2, datum=0.04, x=-0.8766762590444033, y=0.0,
                        r=0.1233237409555968),
            circ.Circle(level=2, datum=0.05, ex={'id': 'a1_1'},
                        x=-0.6154723840806618, y=0.0, r=0.13788013400814464),
            circ.Circle(level=2, datum=0.1, x=0.6664952237042423,
                        y=0.3369290873460549, r=0.2117455702848763),
            circ.Circle(level=2, datum=0.2, x=-0.11288314691830154,
                        y=-0.230392881357073, r=0.2994534572692975),
            circ.Circle(level=2, datum=0.2, x=0.15631936804871832,
                        y=0.30460197676548245, r=0.2994534572692975),
            circ.Circle(level=2, datum=0.3, x=0.5533243963620484,
    ...
    def test_handle_single_value(self):
        ...
        expected = [circ.Circle(1, 0.05, r=1)]
        self.assertEqual(expected, actual)
