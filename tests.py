...
class CircleTest(unittest.TestCase):
    ...
    def test_repr_and_str_with_id_(self):
        ...
        expected = "Circle(x=0.0, y=0.0, r=2.0, level=1, " \
                   "ex={'label': '3'})"
...
class SpecialCases(unittest.TestCase):
    ...
    def test_single_value(self):
        ...
        expected = [circ.Circle(r=1.0)]
        self.assertEqual(actual, expected)

    def test_two_equal_values(self):
        """Two equal circle cases is also trivial."""
        actual = circ.circlify([1.0, 1.0])
        expected = [circ.Circle(x=0.5, r=0.5), circ.Circle(x=-0.5, r=0.5)]
        self.assertEqual(actual, expected)
...
class CountSerieTestCase(TestCaseWithDisplay):
    ...
    def test_circlify(self):
        ...
        expected = [
            circ.Circle(x=0.0, y=0.0, r=1.0, level=0),
            circ.Circle(x=-0.7680630545906644, y=0.13661056172475666,
                        r=0.21988250795031175),
            circ.Circle(x=-0.4586184780594718, y=0.5154819840108337,
                        r=0.2692999739208646),
            circ.Circle(x=0.15910532107887837, y=-0.6704181394216174,
                        r=0.31096082487194077),
            circ.Circle(x=0.15769153632817096, y=0.5438978793053209,
                        r=0.34766477137653345),
            circ.Circle(x=-0.20976457776763055, y=-0.08515409741642607,
                        r=0.3808476754656075),
            circ.Circle(x=0.5824456027453089, y=-0.08515409741642607,
...
class HandleDataTestCase(unittest.TestCase):
    ...
    def test_integer(self):
        ...
        self.assertEqual([circ.Circle(r=42)], actual)

    def test_float(self):
        """Handles float."""
        actual = circ._handle([42.0], 1)
        self.assertEqual([circ.Circle(r=42.0)], actual)
...
class MultiLevelInputTestCase(TestCaseWithDisplay):
    ...
    def test_json_input(self):
        ...
        self.display(actual, [c.ex['id'] if c.ex and 'id' in c.ex else None
                              for c in actual])
        self.assertEqual(expected, actual)
